...
def test_regular_items_decrease_by_one(gilded_rose):
...
def test_quality_goes_up_for_improving_products(
    gilded_rose, name, sell_in, quality, expected_sell_in, expected_quality
):
...
def test_sulfuras_the_immutable(gilded_rose):
...
def test_quality_does_not_increase_past_50(gilded_rose):
...
def test_conjured_items_decrease_in_quality_twice_as_fast(gilded_rose):
